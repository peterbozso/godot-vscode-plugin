...
2. **Main loop in `make_symbol_document()`:** Gave Constructors and Operators their own dedicated sections instead of lumping them with Methods. Added separate accumulator variables (`constructors_index`, `constructors`, `operators_index`, `operators`) and separate switch cases for `SymbolKind.Constructor` and `SymbolKind.Operator`. The rendered doc page now has six section pairs: Properties, Constructors, Constants, Signals, Methods, Operators — each with an index and a descriptions section, matching the structure of the official Godot documentation.
